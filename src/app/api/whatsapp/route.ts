...
// ─── OpenWA session lifecycle (ported from zenith-book reference) ─────────────
...
async function startSessionIfNeeded(session: OpenWaSession): Promise<OpenWaSession> {
...
async function findOpenWaSessionByName(name: string): Promise<OpenWaSession | null> {
  const sessions = await openWaClient.listSessions()
  return sessions.find((session) => session.name === name) ?? null
}

async function deleteOpenWaSessionsByName(name: string, extraSessionId?: string | null) {
