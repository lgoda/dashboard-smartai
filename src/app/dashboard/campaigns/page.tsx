...
export default function CampaignsPage() {
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="h-10 bg-[#222428] rounded w-40 loading" />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[...Array(3)].map((_, i) => <div key={i} className="h-48 bg-[#222428] rounded-xl loading" />)}
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {campaigns.length === 0 ? (
        <div className="bg-[#222428] rounded-xl p-12 border border-[#141517] text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {campaigns.map((c) => {
            return (
              <div key={c.id} className="bg-[#222428] rounded-xl border border-[#141517] hover:border-[#F59E0B]/30 transition-colors flex flex-col">
                <Link href={`/dashboard/campaigns/${c.id}`} className="flex-1 p-5 block">
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-3 border-t border-[#141517]">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-[#18191C] rounded-2xl border border-[#222428] shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {CAMPAIGN_TYPES.map((t) => (
                    <button key={t.value} type="button" onClick={() => setForm((f) => ({ ...f, type: t.value }))}
                      className={`px-3 py-1.5 rounded-lg text-sm font-medium border transition-colors ${form.type === t.value ? 'border-[#F59E0B] text-[#F59E0B] bg-[#F59E0B]/10' : 'border-[#141517] text-gray-400 hover:border-gray-500'}`}>
                      {t.label}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <textarea value={form.notes} onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))}
                  rows={2} className="w-full px-4 py-2 bg-[#141517] border border-[#141517] rounded-lg text-white placeholder-gray-500 focus:ring-2 focus:ring-[#F59E0B] focus:border-[#F59E0B] resize-none" />
              </div>
              <div>
            ...
            <div className="flex gap-3 p-6 border-t border-[#222428]">
              <button onClick={() => setShowModal(false)} className="flex-1 px-4 py-2.5 bg-[#141517] text-gray-300 rounded-lg font-medium hover:bg-[#18191C] transition-colors">Annulla</button>
