...
import { Megaphone, Trash2 } from '@/app/components/icons'
...
const CAMPAIGN_TYPES = [
  ...
  { value: 'other', label: 'Altro', color: '#F59E0B' },
]
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="h-10 bg-[var(--surface)] rounded w-40 loading" />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[...Array(3)].map((_, i) => <div key={i} className="h-48 bg-[var(--surface)] rounded-xl loading" />)}
        </div>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center space-x-3">
          <div className="w-10 h-10 bg-[#F59E0B] rounded-xl flex items-center justify-center">
            <Megaphone className="w-5 h-5 text-[#1b1d20]" />
          </div>
          <div>
            <h1 className="font-display text-3xl font-bold text-white">Campagne</h1>
            <p className="text-gray-300 mt-1">Gestisci le tue campagne di contatto</p>
          </div>
        </div>
        <button onClick={() => setShowModal(true)}
          className="flex items-center gap-2 px-5 py-2.5 bg-[#F59E0B] text-[#1e293b] rounded-xl font-semibold hover:bg-[#D97706] transition-colors">
          + Nuova campagna
        </button>
      </div>

      {campaigns.length === 0 ? (
        <div className="bg-[var(--surface)] rounded-xl p-12 border border-[var(--line)] text-center">
          <div className="w-16 h-16 bg-[#141517] rounded-full flex items-center justify-center mx-auto mb-4 border border-[var(--line)]">
            <Megaphone className="w-7 h-7 text-[var(--mute-2)]" />
          </div>
          <p className="font-display text-white font-semibold text-lg mb-2">Nessuna campagna</p>
          <p className="text-gray-400 mb-6">Crea la tua prima campagna per iniziare a gestire le liste contatti</p>
          <button onClick={() => setShowModal(true)}
            className="px-6 py-3 bg-[#F59E0B] text-[#1e293b] rounded-xl font-semibold hover:bg-[#D97706] transition-colors">
            + Nuova campagna
          </button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {campaigns.map((c) => {
            return (
              <div key={c.id} className="bg-[var(--surface)] rounded-xl border border-[var(--line)] hover:border-[#F59E0B]/30 transition-colors flex flex-col">
                <Link href={`/dashboard/campaigns/${c.id}`} className="flex-1 p-5 block">
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-3 border-t border-[var(--line)]">
                    <div><p className="text-xs text-gray-500">Import</p><p className="font-mono text-white font-semibold tabular-nums">{c.campaign_imports?.length ?? 0}</p></div>
                    <div><p className="text-xs text-gray-500">In coda</p><p className="font-mono text-white font-semibold tabular-nums">{(c.queued_count ?? 0).toLocaleString('it-IT')}</p></div>
                    <div><p className="text-xs text-gray-500">Orario</p><p className="font-mono text-white text-sm tabular-nums">{c.send_time_from}-{c.send_time_to}</p></div>
                    <div><p className="text-xs text-gray-500">Limite/giorno</p><p className="font-mono text-white text-sm tabular-nums">{c.daily_limit}</p></div>
                  </div>
                </Link>
                <div className="px-5 pb-4">
                  <button
                    ...
                    className="w-full py-1.5 flex items-center justify-center gap-1.5 text-xs text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded-lg transition-colors disabled:opacity-40"
                  >
                    {deletingId === c.id ? 'Eliminazione...' : <><Trash2 className="w-3.5 h-3.5" /> Elimina campagna</>}
                  </button>
                </div>
              </div>
            )
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-[var(--ink)] rounded-2xl border border-[var(--line)] shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-[var(--line)]">
              <h2 className="font-display text-xl font-bold text-white">Nuova campagna</h2>
            ...
            <div className="p-6 space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {CAMPAIGN_TYPES.map((t) => (
                    <button key={t.value} type="button" onClick={() => setForm((f) => ({ ...f, type: t.value }))}
                      className={`px-3 py-1.5 rounded-lg text-sm font-medium border transition-colors ${form.type === t.value ? 'border-[#F59E0B] text-[#F59E0B] bg-[#F59E0B]/10' : 'border-[var(--line)] text-gray-400 hover:border-gray-500'}`}>
                      {t.label}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <textarea value={form.notes} onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))}
                  rows={2} className="w-full px-4 py-2 bg-[#141517] border border-[var(--line)] rounded-lg text-white placeholder-gray-500 focus:ring-2 focus:ring-[#F59E0B] focus:border-[#F59E0B] resize-none" />
              </div>
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {DAY_OPTIONS.map((d) => (
                    <button key={d.key} type="button" onClick={() => toggleDay(d.key)}
                      className={`w-10 h-10 rounded-lg text-sm font-medium transition-colors ${form.send_days.includes(d.key) ? 'bg-[#F59E0B] text-[#1e293b]' : 'bg-[#141517] text-gray-400 hover:text-white'}`}>
                      {d.label}
                    </button>
                  ))}
                </div>
            ...
            <div className="flex gap-3 p-6 border-t border-[var(--line)]">
              <button onClick={() => setShowModal(false)} className="flex-1 px-4 py-2.5 bg-[#141517] text-gray-300 rounded-lg font-medium hover:bg-[var(--ink)] transition-colors">Annulla</button>
              <button onClick={handleCreate} disabled={isSaving}
                className="flex-1 px-4 py-2.5 bg-[#F59E0B] text-[#1e293b] rounded-lg font-semibold disabled:opacity-50 hover:bg-[#D97706] transition-colors">
